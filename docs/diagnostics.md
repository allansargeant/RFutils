...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/rfutils/` |
| Linux | `$XDG_STATE_HOME/rfutils/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\rfutils\logs\` |

`RFUTILS_LOG_DIR` overrides it. The path is printed on the first line of every
...
`rfutils.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `RFUTILS_LOG`, default `info`.
...
Writes `rfutils-diagnostics-<timestamp>.json` and prints its path. One file, so
