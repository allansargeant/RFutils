# Running unsigned RFutils builds
...
RFutils listens on the network, so the first time it starts Windows shows:

> **Windows Defender Firewall has blocked some features of RFutils**
...
domain. RFutils needs this to serve the web UI and receive receiver telemetry and AES67 audio.

If you deny it (or the prompt appears behind another window and times out), Monitor will find no receivers; the offline Convert and Coordinate tools still work
...
**Private**. If RFutils isn't listed, **Allow another app…** → **Browse** to the `.exe`.

Or from an elevated PowerShell:

```powershell
New-NetFirewallRule -DisplayName "RFutils" -Direction Inbound -Program "C:\Path\To\RFutils.exe" -Action Allow -Profile Private,Domain
```

To remove a bad rule and get the prompt back on next launch:

```powershell
Get-NetFirewallRule -DisplayName "*RFutils*" | Remove-NetFirewallRule
```
...
```sh
codesign --force --deep --sign - "/Applications/RFutils.app"
```

Verify it took:

```sh
codesign -dv --verbose=4 "/Applications/RFutils.app"
spctl -a -vv "/Applications/RFutils.app"   # still reports "rejected" — ad-hoc is not notarization
```
